...
def draw_roc_pr(fpr_1, tpr_1, recall_1, precision_1, roc_auc_1, pr_auc_1,
    ...
    plt.plot(fpr_4, tpr_4, 'orange',label='DeepD2V = %0.3f'% roc_auc_4)
    plt.plot(fpr_5, tpr_5, 'c',label='DeepSEA = %0.3f'% roc_auc_5)
    plt.plot(fpr_1, tpr_1, 'b',label='CBR-KAN = %0.3f'% roc_auc_1)
    ...
    plt.plot(recall_4, precision_4, 'orange',label='DeepD2V = %0.3f'% pr_auc_4)
    plt.plot(recall_5, precision_5, 'c',label='DeepSEA = %0.3f'% pr_auc_5)
    plt.plot(recall_1, precision_1, 'b',label='CBR-KAN = %0.3f'% pr_auc_1)
    plt.legend(loc='lower right')
    plt.plot([0,1],[0,1],'r--')
    plt.xlim([0,1])
    plt.ylim([0,1])
...
if __name__ == "__main__":
    ...
    path_res = '../res/existing_methods/'
    fpr_1, tpr_1, recall_1, precision_1, roc_auc_1, pr_auc_1 = get_values(np.load(path_res+ 'CR-KAN_11-19_17.56_0.9539987153921415.npz'))# CBR-KAN
    fpr_2, tpr_2, recall_2, precision_2, roc_auc_2, pr_auc_2 = get_values(np.load(path_res+ 'DeepBind_11-20_09.51_0.8965849309063667.npz'))# DeepBind
    fpr_3, tpr_3, recall_3, precision_3, roc_auc_3, pr_auc_3 = get_values(np.load(path_res+ 'DanQ_11-20_11.11_0.9091356530961291.npz'))# DanQ
    fpr_4, tpr_4, recall_4, precision_4, roc_auc_4, pr_auc_4 = get_values(np.load(path_res+ 'DeepD2V_11-20_13.28_0.9269618851792677.npz'))# DeepD2V
    fpr_5, tpr_5, recall_5, precision_5, roc_auc_5, pr_auc_5 = get_values(np.load(path_res+ 'DeepSEA_11-20_15.20_0.938324488727796.npz'))# DeepSEA
