...
if __name__ == "__main__":
    path1 = '../res/exist50/DeepBind_res.txt'
    path2 = '../res/exist50/DanQ_res.txt'
    path3 = '../res/exist50/DeepD2V_res.txt'
    path4 = '../res/exist50/DeepSEA_res.txt'
    path5 = '../res/exist50/CR-KAN_res.txt' # CR-KAN

    DeepBind_res = get_res(path1)
    DanQ_res = get_res(path2)
    DeepD2V_res = get_res(path3)
    DeepSEA_res = get_res(path4)
    att_res = get_res(path5)# CR-KAN
    ...
    result.index = ['DeepBind','DanQ', 'DeepD2V', 'DeepSEA', 'CR-KAN']
    # print(result)
    # result.to_csv('../res/res_table_mean.csv')
    res_plt = result[["mcc", "f1", "acc",  "roc_auc"]]
    tick_label=['DeepBind','DanQ', 'DeepD2V', 'DeepSEA', 'CBR-KAN']
    width = 0.42
    x = np.arange(0,10,2)

    fig = plt.figure(figsize = (14,8))
    plt.bar(x, res_plt.loc[:,'mcc'],width, label='MCC',color="c",alpha=0.35)
    for i, j in zip(x, res_plt.loc[:,'mcc']):
        plt.text(i+0.001,j+0.001, '%.3f'%j, ha='center')

    plt.bar(x+ width, res_plt.loc[:,'f1'], width, label='F1-score',color="b", alpha=0.35)
    for i, j in zip(x+ width, res_plt.loc[:,'f1']):
        plt.text(i+0.001,j+0.001, '%.3f'%j, ha='center')

    plt.bar(x+ 2*width, res_plt.loc[:,'acc'], width,label='Accuracy', color="y",alpha=0.35)
    for i, j in zip(x+ 2*width, res_plt.loc[:,'acc']):
        plt.text(i+0.001,j+0.001, '%.3f'%j, ha='center')
