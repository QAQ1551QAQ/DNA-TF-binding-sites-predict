...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.cnn2 = nn.Sequential(
            nn.Conv1d(in_channels=100,out_channels=128*3,kernel_size=11),
            nn.ReLU(True),nn.Dropout(0.2),
            nn.MaxPool1d(2),
            nn.Conv1d(in_channels=128*3,out_channels=256,kernel_size=9),
            nn.ReLU(True),nn.Dropout(0.2),
            nn.MaxPool1d(2),
        )

        self.cnn1 = nn.Sequential(
            nn.Conv1d(in_channels=100,out_channels=160*2,kernel_size=9),
            nn.ReLU(True),nn.Dropout(0.2),
            nn.MaxPool1d(2),
            nn.Conv1d(in_channels=160*2,out_channels=160*2,kernel_size=1),
            nn.ReLU(True),nn.Dropout(0.2),
            nn.MaxPool1d(2),
            nn.Conv1d(in_channels=160*2,out_channels=160*2,kernel_size=5),
            nn.ReLU(True),nn.Dropout(0.2),
            nn.MaxPool1d(2),
            nn.Conv1d(in_channels=160*2,out_channels=256,kernel_size=8),
            nn.ReLU(True),nn.Dropout(0.2),
            nn.MaxPool1d(2),
        )
    ...
    def forward(self, x):
        ...
        # CNN
#         out = self.cnn1(self.dropout(out)).permute(0, 2, 1) # [batch_size, seq_len, embedding_dim]
        out = self.cnn2(self.dropout(out)).permute(0, 2, 1)
