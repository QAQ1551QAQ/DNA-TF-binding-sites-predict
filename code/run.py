...
if __name__ == '__main__':
    ...
    one_of_data = pd.read_csv('../data/processed/Gm12878BatfPcr1xPkRep1.csv', encoding='utf-8')
    ...
    k = 0; roc_auc_all=0; pr_auc_all_all=0; pre_all=0; rec_all=0; test_acc_all=0;f1_all=0
    ...
    for train_index, test_index in kf.split(text, label):
        ...
        roc_auc, pr_auc, pre, rec, test_acc, f1 = train(config, model, train_iter, dev_iter, test_iter)

        k += 1; roc_auc_all += roc_auc; pr_auc_all_all += pr_auc; pre_all+=pre; rec_all+=rec; test_acc_all+=test_acc;f1_all+=f1
         # test结果写入文件
    ave_roc_auc = roc_auc_all/k; ave_pr_auc=pr_auc_all_all/k; ave_pre=pre_all/k; ave_rec=rec_all/k; ave_test_acc=test_acc_all/k; ave_f1=f1_all/k
    with open(config.save_res, "a", encoding="utf-8") as fw:
        fw.write(('%s,roc_auc: ,{},  pr_auc: ,{}, pre: ,{}, rec: ,{}, acc: ,{}, f1: ,{},'%file).format(ave_roc_auc, ave_pr_auc, ave_pre, ave_rec, ave_test_acc, ave_f1))
